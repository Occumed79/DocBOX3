...
export default function GridPivotLab() {
  ...
  function gridKey(event:React.KeyboardEvent<HTMLInputElement>,rowIndex:number,columnIndex:number){const input=event.currentTarget;if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='c'){void navigator.clipboard?.writeText(input.value);return}if(event.key==='Escape'){input.blur();return}const moves:Record<string,[number,number]>={ArrowUp:[-1,0],ArrowDown:[1,0],ArrowLeft:[0,-1],ArrowRight:[0,1],Enter:[1,0]};const move=moves[event.key];if(!move)return;event.preventDefault();const target=document.querySelector<HTMLInputElement>(`[data-grid-cell="${rowIndex+move[0]}:${columnIndex+move[1]}"]`);target?.focus();target?.select()}
